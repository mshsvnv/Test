...
func (s *FeedbackService) DeleteFeedback(ctx context.Context, req *dto.DeleteFeedbackReq) error {

	s.logger.Infof("delete feedback, userID, racketID", req.UserID, req.RacketID)
	...
	if err != nil {
		s.logger.Errorf("delete feedback fail, error %s", err.Error())
		return fmt.Errorf("delete feedback fail, error %s", err)
	}
